...
import UploadPFP from "../Popup/UploadPFP";
...
export default function Sidebar() {
    ...
    */
        onAuthStateChanged(auth, (user) => {
            if (user) {
                Fetch("getChats").then((data) => {
                    ...
                    const stringchats = LoadedChats.map(
                        (x: {
                            members: KnownUser[];
                            name: string;
                            id: string;
    ...
    const Styledchats = chats.map((x: Chat) => (
        <ChatCard key={x.id} chatDetails={x} />
    ));
    ...
    return (
        <div className="Sidebar-Wrapper">
            ...
            <div className="SideBar-Chats">{Styledchats}</div>
